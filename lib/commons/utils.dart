...
class Utils{
  ...
  static Future<MyProfileData> updateLikeCount(DocumentSnapshot data, bool isLikePost,MyProfileData myProfileData,ValueChanged<MyProfileData> updateMyData, bool isThread) async {
    ...
    MyProfileData myNewProfileData = MyProfileData(
        ...
        myDisLikeList: isThread ? newLikeList : myProfileData.myDisLikeList,
    ...
    if (isThread) {
      await FBCloudStore.likeToPost(data['catID'], myProfileData,isLikePost);
    }
  ...
  static Future<MyProfileData> updateDislikeCount(DocumentSnapshot data, bool isDisLikePost,MyProfileData myProfileData,ValueChanged<MyProfileData> updateMyData, bool isThread) async {
    ...
    MyProfileData myNewProfileData = MyProfileData(
        myName: myProfileData.myName,
        myThumbnail: myProfileData.myThumbnail,
        myLikeList: isThread ? newDisLikeList : myProfileData.myLikeList,
        myDisLikeList: isThread ? newDisLikeList : myProfileData.myDisLikeList,
        myLikeCommnetList: isThread ? myProfileData.myLikeCommnetList : newDisLikeList
    );
    updateMyData(myNewProfileData);
    isThread ? await FBCloudStore.updatePostDisLikeCount(data,isDisLikePost,myProfileData) : await FBCloudStore.updateCommentLikeCount(data,isDisLikePost,myProfileData);
    if (isThread) {
      await FBCloudStore.dislikeToPost(data['postID'], myProfileData,isDisLikePost);
    }
    return myNewProfileData;
  }
