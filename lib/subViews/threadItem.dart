...
class _ThreadItem extends State<ThreadItem>{
  ...
  void initState() {
    ...
    _DislikeCount = widget.data['postDisLikeCount'];
    super.initState();
  }
...
isDisabled=false;
  ...
  void _updateDisLikeCount(bool isDisLikePost) async{
    MyProfileData _xnewProfileData = await Utils.updateDislikeCount(widget.data,widget.myData.myDisLikeList != null && widget.myData.myDisLikeList.contains(widget.data['postID']) ? true : false,widget.myData,widget.updateMyDataToMain,true);
    setState(() {
      _currentMyData = _xnewProfileData;
    });
    setState(() {
      if(isDisLikePost){
        isDisabled1=true;
...
if(isDisabled1==true||isDisabled1==null&&widget.isFromThread){
  _updateLikeCount(_currentMyData.myLikeList != null &&
...
print(isDisabled);}
                            ...
                            Padding(
                              ...
                              child: Text('Like ( ${widget.isFromThread? widget.data['postLikeCount'] : _likeCount} )',
                                style: TextStyle(fontSize: 16,
                                    ...
                                    color: widget.myData.myLikeList.contains(widget.data['postID'])==true&&isDisabled==false? Colors.blue[900] : Colors.black),),
                            ),
                          ],
                        ),
                      ),
                      GestureDetector(
                       onTap: () {
if(isDisabled==true||isDisabled==null&&widget.isFromThread) {
  _updateDisLikeCount(widget.myData.myDisLikeList != null &&
      widget.myData.myDisLikeList.contains(widget.data['postID'])
      ? true
      : false);
}
                        ...
                        child: Row(
                          children: <Widget>[
                            Icon(Icons.thumb_down,size: 18,color:widget.myData.myDisLikeList.contains(widget.data['postID'])==true &&isDisabled1==false? Colors.blue[900] : Colors.black),
                            Padding(
                              padding: const EdgeInsets.only(left:8.0),
                              child: Text('Dislike ( ${widget.isFromThread ? widget.data['postDisLikeCount'] : _DislikeCount} )',
                                style: TextStyle(fontSize: 16,
                                    fontWeight: FontWeight.bold,
                                    color:widget.myData.myDisLikeList.contains(widget.data['postID'])==true &&isDisabled1==false ? Colors.blue[900] : Colors.black),),
